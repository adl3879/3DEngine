...
{
    // Print to the TTY
    LOG_CORE_INFO("{}:{}: ({}) {}", inFile, inLine, inExpression, (inMessage != nullptr ? inMessage : ""));
...
namespace Physics
{
DynamicWorld::DynamicWorld(Scene *scene)
    : m_Scene(scene)
{
...
void DynamicWorld::AddRigidBody(RigidBodyRef rb)
{
    ...
    bodySettings.mUserData = static_cast<uint64_t>(rb->GetEntity().GetEntityID());
...
JPH::Ref<JPH::Shape> DynamicWorld::GetJoltShape(const PhysicShapeRef shape)
{
    ...
    switch (shape->GetType())
    {
        ...
        {
            auto *meshShape = reinterpret_cast<MeshShape *>(shape.get());
        ...
        {
            auto *convexHullShape = reinterpret_cast<Physics::ConvexHullShape *>(shape.get());
...
void DynamicWorld::DrawDebug(const glm::mat4 &projection, const glm::mat4 &view, Entity entity)
{
